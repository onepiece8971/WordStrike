...
class studybak
{
    ...
    public function randOneWord($books_id)
    {
        ...
        $query_recite = "SELECT words_id FROM ".Wordstrike::$table_prefix
            ."recite WHERE uid = ".$this->uid." AND act != 0";
        ...
        if ($count == 0) { //没有已背单词
            $query_words_id = "SELECT words_id FROM ".Wordstrike::$table_prefix
                ."words_books_words WHERE books_id = ".$books_id
                ." ORDER BY rand() limit 1";
        } else {
            $query_words_id = "SELECT words_id FROM ".Wordstrike::$table_prefix
                ."words_books_words WHERE books_id = ".$books_id
                ." AND words_id NOT IN (".$words_ids
                .") ORDER BY rand() limit 1";
        }
    ...
    {
        global $wpdb;
        $query = "SELECT * FROM ".Wordstrike::$table_prefix
            ."recite WHERE words_id = ".$words_id." AND uid = ".$this->uid;
    ...
    {
        global $wpdb;
        $query_one = "SELECT id, word_name, means, part, phonetic, voice FROM "
            .Wordstrike::$table_prefix."words WHERE id = ".$words_id
            ." AND act = 1";
    ...
    public function getOneReviewWord()
    {
        global $wpdb;
        $now = time();
        $query = "SELECT words_id FROM ".Wordstrike::$table_prefix
            ."recite WHERE uid = ".$this->uid." AND ".$now
            ." - update_time >= level_time AND act = 1 order by update_time LIMIT 1";
    ...
    public function getOneReviewWordAfterBegin($begin)
    {
        ...
        $query = "SELECT words_id FROM ".Wordstrike::$table_prefix
            ."recite WHERE uid = ".$this->uid." AND ".$now
            ." - update_time >= level_time AND create_time > ".$begin
            ." AND act = 1 order by update_time LIMIT 1";
        $words_id = $wpdb->get_var($query);
        return $this->getWordById($words_id);
    }
    ...
    {
        global $wpdb;
        $query = "SELECT `level` FROM ".Wordstrike::$table_prefix
            ."recite WHERE words_id = ".$words_id." AND uid = ".$this->uid
            ." AND act = 1";
    ...
    public function getTodayReciteWordCount()
    {
        ...
        $query = "SELECT count(1) FROM ".Wordstrike::$table_prefix
            ."recite WHERE uid = ".$this->uid."  AND create_time >= ".$today
            ." AND act = 1";
